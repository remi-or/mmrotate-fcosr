...
class FCOSRboxHead(BaseDenseHead):
    ...
    def __init__(
        ...
        self.qlf_on = cls_loss['type'] == 'QualityFocalLoss'
        self.use_iou = (self.qlf_on and self.cls_scores
    ...
    def build_regression_subnetwork(self, stacked_convs: int, conv_cfg: dict,
                                    norm_cfg: dict, nstrides: int) -> None:
        ...
        Args:
            ...
            conv_cfg (dict, optional): Config dict for convolution layer.
            norm_cfg (dict, optional): Config dict for normalization layer.
            nstrides (int): The number of strides.
        ...
        self.reg_scales = nn.ModuleList([Scale() for _ in range(nstrides)])
    ...
    def forward_single_lvl(self, feature_map: Tensor, stride: Tensor,
        ...
        reg_theta = torch.fmod(self.reg_theta_conv(reg_features), HALF_PI)
    ...
    def forward(
        ...
        cls_logits, pred_rboxes = self.forward_multi_lvl(feature_maps)
        pred_rboxes[:, :, :2] += points
        return cls_logits, pred_rboxes, points, strides, regranges
    ...
    def forward_train(self, feature_maps: Tuple[Tensor,
        ...
        # head forward pass
        cls_logits, pred_boxes, points, strides, regranges = self.forward(
            feature_maps)
        # compute targets for all images
        cls_targets, cls_scores, reg_targets, reg_wghts = self.compute_targets(
            gt_boxes, gt_labels, pred_boxes, points, strides, regranges)
        # format predictions
    ...
    def loss(self, cls_logits: Tensor, cls_targets: Tensor, cls_scores: Tensor,
        ...
        avg_factor = max(1.0, reg_weights.size(0))
        if self.qlf_on:
    ...
    def get_bboxes_single(
        ...
        if nms_pre > 0:
            kept = torch.zeros_like(strides, dtype=torch.bool)
            for lvl_stride in self.strides:
                lvl_mask = lvl_stride == strides  # P
                if lvl_mask.sum() > nms_pre:
                    lvl_max_scores = cls_scores[lvl_mask].max(dim=1)[0]  # L
                    lvl_topk_inds = lvl_max_scores.topk(nms_pre)[1]  # k
                    kept[lvl_mask.nonzero()[lvl_topk_inds]] = 1
                else:
                    kept[lvl_mask] = 1
        ...
        if rescale:
            rbox_preds[:, 0:4:2] /= scale_factor[0]
            rbox_preds[:, 1:4:2] /= scale_factor[1]
        # apply NMS
    ...
    def get_bboxes(self,
        ...
        dets_list, labels_list = multi_apply(self.get_bboxes_single,
                                             ...
                                             repeat(self.test_cfg),
                                             repeat(rescale))
        # check if the batch is a rotation test
        rotations = self.test_cfg.get('test_rotations', [])
        ...
        dets, labels, _ = multiclass_nms_rotated_extra(
            ...
            nms=self.test_cfg.get('nms', 0.1),
            extra_nms=self.test_cfg.get('extra_nms'),
            max_num=self.test_cfg.get('max_per_img', 2000))
        return [(dets, labels)]
