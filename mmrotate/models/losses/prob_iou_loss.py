...
def gbb_form(boxes):
    return torch.cat(
        (boxes[:, :2], torch.pow(boxes[:, 2:4], 2) / 12.0, boxes[:, 4:]), 1)
...
def probiou_loss(pred, target, eps=1e-3, mode='l1'):
    """
        pred    -> a matrix [N,5](x,y,w,h,angle - in radians)
            containing ours predicted box in case of HBB angle == 0
        target  -> a matrix [N,5](x,y,w,h,angle - in radians)
            containing ours target box ;in case of HBB angle == 0
        eps     -> threshold to avoid infinite values
        mode    -> ('l1' in [0,1] or 'l2' in [0,inf]) metrics
            according our paper
    """
    ...
    x1, y1 = gbboxes1[:, 0], gbboxes1[:, 1],
    a1_, b1_, c1_ = gbboxes1[:, 2], gbboxes1[:, 3], gbboxes1[:, 4]
    x2, y2 = gbboxes2[:, 0], gbboxes2[:, 1]
    a2_, b2_, c2_ = gbboxes2[:, 2], gbboxes2[:, 3], gbboxes2[:, 4]

    a1, b1, c1 = rotated_form(a1_, b1_, c1_)
    a2, b2, c2 = rotated_form(a2_, b2_, c2_)

    t1 = 0.25 * ((a1 + a2) * (torch.pow(y1 - y2, 2)) +
                 (b1 + b2) * (torch.pow(x1 - x2, 2))) + \
...
class ProbiouLoss(nn.Module):
    ...
    def forward(self, loc_p, loc_t, weight, avg_factor=None):
        if avg_factor is None:
            avg_factor = 1.0
        loss = self.loss_weight * torch.sum(
            probiou_loss(loc_p, loc_t, self.eps, self.mode) *
